...
http.interceptors.request.use((cfg) => {
  const t = localStorage.getItem('tf_admin_token')
...
function activityDisplayTitle(activity) {
  if (!activity) return ''
  const className = activity.classroom?.name || '未识别班级'
...
function adminConfirm(message, title = '确认') {
...
const App = {
  setup() {
    ...
    const token = ref(localStorage.getItem('tf_admin_token') || '')
    ...
    const navItems = computed(() => NAV_ITEMS.map((item) => {
      const translated = ADMIN_I18N[language.value]?.nav?.[item.key] || ADMIN_I18N['zh-CN'].nav[item.key]
    ...
    function setLanguage(lang) {
      ...
      updateDocumentTitle()
      nextTick(() => translateVisibleText(document.getElementById('app'), language.value, ADMIN_TEXT_I18N))
    }
    ...
    onMounted(() => {
      ...
      const observer = new MutationObserver(() => {
        ...
        observer._tfTimer = window.setTimeout(() => {
          translateVisibleText(document.getElementById('app'), language.value, ADMIN_TEXT_I18N)
        }, 30)
      })
      observer.observe(document.getElementById('app'), { childList: true, subtree: true })
      window.__teammindAdminI18nObserver = observer
    ...
    onUnmounted(() => {
      stopAutoRefresh()
      window.__teammindAdminI18nObserver?.disconnect()
      window.removeEventListener('hashchange', onHashChange)
  ...
  template: `
    ...
    <div v-else class="layout">
      ...
      <div class="main">
        ...
        <main class="content">
          <template v-if="page==='classes'">
            <div class="activity-hero">
              <div>
                ...
                <h2>班级管理</h2>
                <p>先维护班级和成员，再在组队活动中选择班级范围。学生加入或退出班级都需要老师审批，避免分组范围混乱。</p>
              </div>
              <div class="hero-stats">
                <div><strong>{{ classes.length }}</strong><span>班级数</span></div>
                <div><strong>{{ classes.reduce((s,c)=>s+(c.member_count||0),0) }}</strong><span>班级学生</span></div>
                <div><strong>{{ classes.reduce((s,c)=>s+(c.pending_count||0),0) }}</strong><span>待审批</span></div>
                <div><strong>{{ classAdvice.group_count || 0 }}</strong><span>建议组数</span></div>
              </div>
            </div>

            <div class="class-layout">
              <div class="card class-list-card">
                <div class="card-header"><h3>班级列表</h3><el-button @click="loadClasses">刷新</el-button></div>
                <div v-if="!classes.length" class="empty-note">暂无班级，请先创建。</div>
                <div v-for="c in classes" :key="c.id" class="activity-row" :class="{active: selectedClassId===c.id}" @click="loadClassDetail(c.id)">
                  <div>
                    <strong>{{ c.name }}</strong>
                    <p>{{ c.course_name || c.major || '未设置课程' }} · {{ formatStatus(c.status) }}</p>
                  </div>
                  <div style="display:flex;gap:6px;align-items:center">
                    <el-tag>{{ c.member_count || 0 }} 人</el-tag>
                    <el-tag v-if="c.pending_count" type="warning">{{ c.pending_count }} 审批</el-tag>
                  </div>
                </div>
              </div>
              ...
              <div class="card class-main-card">
                <div class="card-header">
                  <div>
                    <h3>{{ selectedClass?.name || '新建班级' }}</h3>
                    <p class="hint">{{ selectedClass?.grouping_advice?.summary || '一个班建议 10-20 人，系统会按人数给出均匀分组方案。' }}</p>
                  </div>
                  <el-button type="primary" plain @click="createClass">创建为新班级</el-button>
                </div>
                <el-tabs v-model="classTab">
                  <el-tab-pane label="成员" name="members">
                    <div class="class-tools">
                      <el-select v-model="classMemberForm.user_ids" multiple filterable placeholder="选择要拉入的学生" style="width:100%">
                        <el-option v-for="row in classAvailableUsers" :key="row.user.id" :label="row.user.name + ' / ' + row.user.account" :value="row.user.id" />
                      </el-select>
                      <el-button type="primary" @click="addClassMembers">拉入同学</el-button>
                    </div>
                    <el-table :data="classMembers" stripe empty-text="当前班级暂无学生">
                      <el-table-column label="姓名"><template #default="{row}">{{ row.user?.name }}</template></el-table-column>
                      <el-table-column label="账号"><template #default="{row}">{{ row.user?.account }}</template></el-table-column>
                      <el-table-column label="画像"><template #default="{row}"><el-tag :type="row.profile?'success':'info'" size="small">{{ row.profile ? '已录入' : '未录入' }}</el-tag></template></el-table-column>
                      <el-table-column label="角色方向"><template #default="{row}">{{ row.profile?.pref_role || '—' }}</template></el-table-column>
                      <el-table-column label="操作" width="110"><template #default="{row}"><el-button size="small" type="danger" plain @click="removeClassMember(row)">剔除</el-button></template></el-table-column>
                    </el-table>
                  </el-tab-pane>

                  <el-tab-pane :label="'班级活动' + (classActivities.length ? ' (' + classActivities.length + ')' : '')" name="activities">
                    <p class="hint">这里展示当前班级关联的组队活动。教师在活动中心创建活动时选择班级后，会自动出现在这里；学生从学生端发起的班级活动也会同步显示。</p>
                    <el-table :data="classActivities" stripe empty-text="当前班级暂无活动">
                      <el-table-column prop="title" label="活动标题" min-width="180" />
                      <el-table-column label="方式" width="130"><template #default="{row}">{{ formatMode(row.mode, true) }}</template></el-table-column>
                      <el-table-column label="状态" width="110"><template #default="{row}">{{ formatStatus(row.status) }}</template></el-table-column>
                      <el-table-column label="参与/小组" width="130"><template #default="{row}">{{ row.participant_count || 0 }} 人 / {{ row.group_count || row.team_count || 0 }} 组</template></el-table-column>
                      <el-table-column label="操作" width="130">
                        <template #default="{row}">
                          <el-button size="small" type="primary" plain @click="go('workbench'); loadActivityDetail(row.id)">查看活动</el-button>
                        </template>
                      </el-table-column>
                    </el-table>
                  </el-tab-pane>

                  <el-tab-pane :label="'申请审批' + (classPendingRequests.length ? ' (' + classPendingRequests.length + ')' : '')" name="requests">
                    <el-table :data="classRequests" stripe empty-text="暂无申请">
                      <el-table-column label="学生"><template #default="{row}">{{ row.user?.name || row.user_id }}</template></el-table-column>
                      <el-table-column label="类型" width="90"><template #default="{row}">{{ formatRequestType(row.request_type) }}</template></el-table-column>
                      <el-table-column label="状态" width="100"><template #default="{row}"><el-tag :type="row.status==='pending'?'warning':(row.status==='approved'?'success':'info')" size="small">{{ formatStatus(row.status) }}</el-tag></template></el-table-column>
                      <el-table-column prop="message" label="申请说明" />
                      <el-table-column label="操作" width="180">
                        <template #default="{row}">
                          <el-button size="small" type="success" :disabled="row.status!=='pending'" @click="reviewClassRequest(row,'approve')">通过</el-button>
                          <el-button size="small" plain :disabled="row.status!=='pending'" @click="reviewClassRequest(row,'reject')">拒绝</el-button>
                        </template>
                      </el-table-column>
                    </el-table>
                  </el-tab-pane>

                  <el-tab-pane label="分组建议" name="advice">
                    <div class="class-advice-box">
                      <div>
                        <h3>{{ classAdvice.summary || '请选择班级查看建议' }}</h3>
                        <p class="hint">建议基于当前 active 成员人数生成；真正自动分组时还会结合画像均衡。</p>
                      </div>
                      <div class="class-tools compact">
                        <el-input-number v-model="classAdviceGroupSize" :min="3" :max="5" />
                        <el-button @click="refreshClassAdvice">重新计算</el-button>
                      </div>
                    </div>
                    <div v-if="classAdvice.ai_analysis" class="ai-insight-card">
                      <div class="ai-insight-head">
                        <span>AI</span>
                        <div>
                          <strong>智能分组分析</strong>
                          <p>{{ classAdvice.ai_analysis.source==='deepseek' ? 'DeepSeek 大模型分析' : '规则兜底分析（未配置大模型或调用失败）' }}</p>
                        </div>
                      </div>
                      <p>{{ classAdvice.ai_analysis.summary }}</p>
                      <div class="ai-insight-grid">
                        <div><b>分析依据</b><span v-for="x in classAdvice.ai_analysis.rationale||[]" :key="x">{{ x }}</span></div>
                        <div><b>教师建议</b><span v-for="x in classAdvice.ai_analysis.teacher_actions||[]" :key="x">{{ x }}</span></div>
                        <div><b>风险提醒</b><span v-for="x in classAdvice.ai_analysis.risks||[]" :key="x">{{ x }}</span></div>
                      </div>
                    </div>
                  ...
                  <el-tab-pane label="班级设置" name="settings">
                    <el-form label-position="top" class="class-form-grid">
                      <el-form-item label="班级名称"><el-input v-model="classForm.name" placeholder="如：人工智能 2401 班" /></el-form-item>
                      <el-form-item label="班级代码"><el-input v-model="classForm.code" placeholder="如：AI2401" /></el-form-item>
                      ...
                      <el-form-item label="课程名称"><el-input v-model="classForm.course_name" /></el-form-item>
                      <el-form-item label="人数上限"><el-input-number v-model="classForm.max_students" :min="10" :max="60" /></el-form-item>
                      <el-form-item label="说明" class="wide"><el-input v-model="classForm.description" type="textarea" :rows="3" /></el-form-item>
                    </el-form>
                    <div class="toolbar-row">
                      <el-button type="primary" @click="saveClass">保存当前班级</el-button>
                      <el-button type="danger" plain @click="archiveClass">解散/归档班级</el-button>
                    </div>
                  </el-tab-pane>
                </el-tabs>
              </div>
          ...
          <template v-if="page==='workbench'">
            <div class="activity-hero">
              <div>
                <p class="eyebrow">Team Activity Center</p>
                <h2>组队活动中心</h2>
                <p>先创建一次组队活动，再选择“任务驱动自动组队”或“学生自由组队”。学生只有参与该活动后，才会进入对应队伍流程。</p>
              </div>
              <div class="hero-stats">
                <div><strong>{{ activities.length }}</strong><span>活动数</span></div>
                <div><strong>{{ selectedActivity?.participant_count || 0 }}</strong><span>参与学生</span></div>
                <div><strong>{{ selectedActivity?.group_count || selectedActivity?.team_count || 0 }}</strong><span>队伍/小组</span></div>
                <div><strong>{{ commandDash.feedback_count || 0 }}</strong><span>任务反馈</span></div>
              </div>
            </div>

            <div class="activity-layout">
              <div class="card activity-form-card">
                <div class="card-header"><h3>创建组队活动</h3></div>
                <el-form label-position="top">
                  <el-form-item label="活动标题"><el-input v-model="activityForm.title" placeholder="如：第 4 周产品原型项目组队" /></el-form-item>
                  <el-form-item label="课程/场景"><el-input v-model="activityForm.course_name" placeholder="可选，如：人机交互课程" /></el-form-item>
                  <el-form-item label="班级范围">
                    <el-select v-model="activityForm.class_id" filterable placeholder="请选择班级：活动必须归属于具体班级" style="width:100%">
                      <el-option v-for="c in classes" :key="c.id" :label="c.name + ' · ' + (c.member_count || 0) + '人'" :value="c.id" />
                    </el-select>
                    <p class="hint" v-if="classes.find(c=>c.id===activityForm.class_id)?.grouping_advice">
                      {{ classes.find(c=>c.id===activityForm.class_id).grouping_advice.summary }}
                    </p>
                  </el-form-item>
                  <el-form-item label="组队方式">
                    <div class="mode-card-grid">
                      <div class="mode-card" :class="{active: activityForm.mode==='task_auto'}" @click="activityForm.mode='task_auto'">
                        <strong>任务驱动自动组队</strong>
                        <span>老师填写任务需求，学生补充标签后系统自动匹配。</span>
                      </div>
                      <div class="mode-card" :class="{active: activityForm.mode==='free_team'}" @click="activityForm.mode='free_team'">
                        <strong>学生自由组队</strong>
                        <span>学生创建或加入队伍，老师最后锁定结果。</span>
                      </div>
                    </div>
                  </el-form-item>
                  <el-form-item label="每组人数"><el-input-number v-model="activityForm.group_size" :min="2" :max="8" /></el-form-item>
                  <el-form-item label="任务需求/组队说明"><el-input v-model="activityForm.task_goal" type="textarea" :rows="4" placeholder="说明这次任务要做什么、希望队伍具备哪些能力" /></el-form-item>
                  <el-form-item label="建议角色">
                    <el-select v-model="activityForm.required_roles" multiple filterable allow-create default-first-option placeholder="输入或选择角色" style="width:100%">
                      <el-option v-for="r in ROLE_OPTIONS" :key="r" :label="r" :value="r" />
                    </el-select>
                  </el-form-item>
                  <el-form-item label="关键标签"><el-select v-model="activityForm.required_tags" multiple filterable allow-create default-first-option placeholder="如 Python、数据分析、文档汇报" style="width:100%" /></el-form-item>
                  <el-button type="primary" :loading="loading" @click="createActivity">创建活动</el-button>
                </el-form>
              </div>

              <div class="card activity-list-card">
                <div class="card-header"><h3>活动列表</h3><el-button @click="loadActivities">刷新</el-button></div>
                <div v-if="!activities.length" class="empty-note">暂无活动，先创建一个组队活动。</div>
                <div
                ...
                >
                  <div class="activity-row-main">
                    ...
                    <p>{{ a.classroom?.name || '未识别班级' }} · {{ formatMode(a.mode) }} · {{ formatStatus(a.status) }}</p>
                  </div>
                  <div class="activity-row-meta">
                    <el-tag>{{ a.participant_count || 0 }} 人参与</el-tag>
                    <span class="open-detail-link">打开详情</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div v-if="selectedActivity" class="card activity-detail-card">
              <div class="card-header">
                <div>
                  ...
                  <p class="hint">{{ selectedActivity.task_goal || selectedActivity.description || '暂无任务说明' }}</p>
                </div>
                <el-tag size="large" type="success">{{ formatMode(selectedActivity.mode, true) }} · {{ formatStatus(selectedActivity.status) }}</el-tag>
              </div>
              <div class="activity-actions">
                <el-button type="primary" plain :disabled="selectedActivity.status!=='draft'" :loading="loading" @click="publishActivityCollect">发布给学生填写</el-button>
                <el-button v-if="selectedActivity.mode==='task_auto'" type="primary" :disabled="!activityParticipants.length" :loading="loading" @click="autoGroupActivity">生成候选分组</el-button>
                <el-button v-if="selectedActivity.mode==='task_auto'" type="success" :disabled="!activityGroups.length" :loading="loading" @click="publishActivityGroups">锁定正式团队</el-button>
                <el-button v-if="selectedActivity.mode==='free_team'" type="success" :disabled="!activityRooms.length" :loading="loading" @click="lockFreeTeams">锁定自由组队结果</el-button>
              </div>

              <div class="activity-summary-grid">
                <div><span>参与人数</span><strong>{{ activityParticipants.length }}</strong></div>
                <div><span>已生成小组</span><strong>{{ activityGroups.length }}</strong></div>
                <div><span>自由队伍</span><strong>{{ activityRooms.length }}</strong></div>
                <div><span>确认率</span><strong>{{ activityConfirmationSummary.accept_rate || 0 }}%</strong></div>
              </div>
              ...
              <div v-if="selectedActivity.ai_analysis" class="ai-insight-card">
                <div class="ai-insight-head">
                  <span>AI</span>
                  <div>
                    <strong>活动分组智能分析</strong>
                    <p>{{ selectedActivity.ai_analysis.source==='deepseek' ? 'DeepSeek 大模型返回' : '规则兜底分析（未配置大模型或调用失败）' }}</p>
                  </div>
                </div>
                <p>{{ selectedActivity.ai_analysis.summary }}</p>
                <div class="ai-insight-grid">
                  <div><b>专业判断</b><span v-for="x in selectedActivity.ai_analysis.rationale||[]" :key="x">{{ x }}</span></div>
                  <div><b>下一步动作</b><span v-for="x in selectedActivity.ai_analysis.teacher_actions||[]" :key="x">{{ x }}</span></div>
                  <div><b>风险预警</b><span v-for="x in selectedActivity.ai_analysis.risks||[]" :key="x">{{ x }}</span></div>
                </div>
              </div>

              <div class="activity-section">
                <h4>参与学生</h4>
                <el-table :data="activityParticipants" size="small" empty-text="等待学生参与">
                  <el-table-column label="姓名"><template #default="{row}">{{ row.user?.name || row.user_id }}</template></el-table-column>
                  <el-table-column label="标签数"><template #default="{row}">{{ (row.active_tags||[]).length }}</template></el-table-column>
                  <el-table-column label="状态"><template #default="{row}"><el-tag size="small">{{ formatStatus(row.status) }}</el-tag></template></el-table-column>
                </el-table>
              </div>

              <div v-if="selectedActivity.mode==='task_auto'" class="activity-section">
                <h4>候选分组结果</h4>
                <el-table :data="activityGroups" size="small" empty-text="尚未生成小组">
                  <el-table-column prop="group_name" label="小组" width="120" />
                  <el-table-column label="成员数" width="90"><template #default="{row}">{{ row.member_ids?.length || 0 }}</template></el-table-column>
                  <el-table-column prop="balance_score" label="均衡度" width="90" />
                  <el-table-column label="确认" width="130"><template #default="{row}">{{ row.confirmation_summary?.accepted || 0 }}/{{ row.confirmation_summary?.total || 0 }} · {{ row.confirmation_summary?.accept_rate || 0 }}%</template></el-table-column>
                  <el-table-column label="说明"><template #default="{row}"><span class="explain-text">{{ row.ai_analysis?.summary || row.complement_note || '—' }}</span></template></el-table-column>
                </el-table>
              </div>

              <div v-if="selectedActivity.mode==='task_auto' && activityGroups.length" class="activity-section">
                <h4>预沟通确认与微调申请</h4>
                <p class="hint">学生确认后再锁定正式团队；有微调申请时，老师可记录处理意见并同步调整角色。</p>
                <el-table :data="activityConfirmations" size="small" empty-text="等待学生确认">
                  <el-table-column label="学生" width="110"><template #default="{row}">{{ row.user?.name || row.user_id }}</template></el-table-column>
                  ...
                  <el-table-column prop="preferred_role" label="期望/推荐角色" width="140" />
                  <el-table-column label="任务偏好" min-width="160"><template #default="{row}">{{ formatTagList(row.task_preferences) }}</template></el-table-column>
                  <el-table-column prop="reason" label="原因/备注" min-width="180" />
                  <el-table-column prop="handled_note" label="处理说明" min-width="160" />
                  <el-table-column label="操作" width="180">
                    <template #default="{row}">
                      <el-button size="small" type="primary" plain @click="resolveConfirmation(row,'resolved')">处理</el-button>
                      <el-button size="small" plain @click="resolveConfirmation(row,'rejected')">驳回</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              </div>
              ...
              <div v-if="selectedActivity.mode==='free_team'" class="activity-section">
                ...
                <div class="free-room-grid">
                  <div v-for="room in activityRooms" :key="room.id" class="free-room-card">
                    ...
                    <p>{{ room.description || '暂无队伍说明' }}</p>
                    <div class="room-members">
                      <el-tag v-for="m in room.members||[]" :key="m.id" size="small">{{ m.name }}</el-tag>
                    </div>
                    <p class="hint">待处理申请：{{ (room.pending_requests||[]).length }}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div v-if="selectedActivity" class="activity-drawer-content">
                <div class="activity-drawer-head">
                  <div>
                    ...
                    <p>{{ selectedActivity.task_goal || selectedActivity.description || '暂无任务说明' }}</p>
                  </div>
                  <el-button plain @click="activityDetailDrawerVisible=false">关闭</el-button>
                </div>
                ...
                <div class="activity-drawer-tags">
                  ...
                  <el-tag type="info">{{ selectedActivity.classroom?.name || '未识别班级' }}</el-tag>
                </div>
                ...
                <div class="activity-drawer-meta">
                  <div><span>班级范围</span><strong>{{ selectedActivity.classroom?.name || '未识别班级' }}</strong></div>
                  <div><span>每组人数</span><strong>{{ selectedActivity.group_size || activityForm.group_size || 4 }} 人</strong></div>
                  <div><span>建议角色</span><strong>{{ formatList(selectedActivity.required_roles) }}</strong></div>
                  <div><span>关键标签</span><strong>{{ formatList(selectedActivity.required_tags) }}</strong></div>
                </div>
                ...
                <div class="activity-section">
                  <h4>参与学生</h4>
                  <el-table :data="activityParticipants" size="small" empty-text="等待学生参与">
                    <el-table-column label="姓名" width="120"><template #default="{row}">{{ row.user?.name || row.user_id }}</template></el-table-column>
                    <el-table-column label="标签"><template #default="{row}">{{ formatTagList(row.active_tags) }}</template></el-table-column>
                    <el-table-column label="状态" width="110"><template #default="{row}"><el-tag size="small">{{ formatStatus(row.status) }}</el-tag></template></el-table-column>
                  </el-table>
                </div>
          ...
          <template v-if="page==='users'">
            <div class="card users-card">
              <div class="card-header">
                <div>
                  <h3>学员画像全景</h3>
                  <p class="hint">教师端展示完整评分、来源拆解、标签证据与分组参考；学生端不展示分数。</p>
                </div>
                <el-button type="primary" plain @click="rulesVisible=true">查看计算规则</el-button>
              </div>
              <div class="teacher-overview-grid">
                <div class="teacher-metric"><span>学员总数</span><strong>{{ studentUsers.length }}</strong></div>
                <div class="teacher-metric"><span>画像已录入</span><strong>{{ profileReady.length }}</strong></div>
                <div class="teacher-metric"><span>画像完成率</span><strong>{{ profileRate }}%</strong></div>
              </div>
              <div class="users-filter-panel">
                <div class="filter-field">
                  <span>活动筛选</span>
                  <el-select v-model="selectedActivityId" clearable placeholder="全部组队活动" @change="loadActivityDetail">
                    <el-option v-for="a in activities" :key="a.id" :label="a.title" :value="a.id" />
                  </el-select>
                </div>
                <div class="filter-field grow">
                  <span>搜索学员</span>
                  <el-input v-model="userSearch" placeholder="输入姓名或账号" clearable />
                </div>
                <div class="filter-summary">
                  当前显示 <strong>{{ filteredUsers.length }}</strong> 人
                </div>
              </div>
              <el-table :data="filteredUsers" stripe class="profile-table" empty-text="暂无符合条件的学员">
                <el-table-column label="姓名"><template #default="{row}">{{ row.user?.name }}</template></el-table-column>
                <el-table-column label="账号"><template #default="{row}">{{ row.user?.account }}</template></el-table-column>
                <el-table-column label="画像" width="100">
                  <template #default="{row}"><el-tag :type="row.has_profile?'success':'info'" size="small">{{ row.has_profile?'已录入':'未录入' }}</el-tag></template>
                </el-table-column>
                <el-table-column label="三维综合分" width="240">
                  <template #default="{row}">
                ...
                <el-table-column label="专业/角色" min-width="160">
                  <template #default="{row}">
                    <div v-if="row.profile" class="profile-brief">
                      <strong>{{ row.profile.major || row.profile.field || '未识别专业' }}</strong>
                      <span>{{ row.profile.pref_role || '未识别角色' }}</span>
                    </div>
                    <span v-else>—</span>
                  </template>
                </el-table-column>
                <el-table-column label="主动标签" min-width="220">
                  <template #default="{row}">
                    <div class="tag-cell">
                ...
                <el-table-column label="被动标签" min-width="220">
                  <template #default="{row}">
                    <div class="tag-cell">
                ...
                <el-table-column label="操作" width="120">
                  <template #default="{row}">
                    <el-button size="small" type="primary" plain :disabled="!row.profile" @click="openProfileDrawer(row)">画像详情</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
          ...
          <template v-if="page==='community'">
            <div class="card">
              <div class="card-header"><h3>学习社区帖子</h3><el-button @click="loadCommunityPosts">刷新</el-button></div>
              <el-table :data="communityPosts" stripe>
                <el-table-column prop="id" label="ID" width="70" />
                <el-table-column prop="author" label="作者" width="120" />
                <el-table-column prop="title" label="标题" width="180" />
                <el-table-column label="内容" min-width="240"><template #default="{row}">{{ row.content?.slice(0,80) }}</template></el-table-column>
                <el-table-column label="互动" width="150"><template #default="{row}">赞{{ row.stats?.likes||0 }} / 藏{{ row.stats?.favorites||0 }} / 评{{ row.stats?.comments||0 }}</template></el-table-column>
                <el-table-column label="状态" width="100"><template #default="{row}">{{ formatStatus(row.status) }}</template></el-table-column>
                <el-table-column label="操作" width="180">
                  <template #default="{row}">
                    <el-button size="small" type="warning" @click="updatePostStatus(row, 'hidden')">隐藏</el-button>
                    <el-button size="small" type="success" @click="updatePostStatus(row, 'published')">恢复</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
          ...
          <template v-if="page==='export'">
            <div class="card">
              <div class="export-grid">
                <div class="export-tile" @click="downloadExport('profile')"><div class="tile-icon">📊</div><div class="tile-label">导出画像</div></div>
                <div class="export-tile" @click="downloadExport('group')"><div class="tile-icon">👥</div><div class="tile-label">导出分组</div></div>
                <div class="export-tile" @click="downloadExport('task')"><div class="tile-icon">📋</div><div class="tile-label">导出任务</div></div>
              </div>
            </div>
          </template>
          ...
          <template v-if="page==='account'">
            <div class="account-hero">
              ...
              <div>
                ...
                <el-tag type="success">管理员账号：{{ user?.account }}</el-tag>
                <el-tag v-if="user?.availability">{{ user.availability }}</el-tag>
              </div>
            </div>
            <div class="profile-showcase">
              <div class="showcase-item"><b>教学/研究方向</b><span>{{ user?.research_interest || '填写课程主题、研究兴趣或负责班级' }}</span></div>
              <div class="showcase-item"><b>课程主页</b><a v-if="user?.portfolio_url" :href="user.portfolio_url" target="_blank">Course / Portfolio</a><span v-else>未填写</span></div>
              <div class="showcase-item"><b>公开主页</b><a v-if="user?.github_url" :href="user.github_url" target="_blank">GitHub / Lab</a><span v-else>未填写</span></div>
            </div>
            <div class="account-grid">
              <div class="card">
                <h3>个人资料</h3>
                <p class="hint">头像支持图片链接；留空时显示姓名首字。</p>
                <el-form label-position="top">
                  <el-form-item label="姓名"><el-input v-model="accountForm.name" maxlength="64" /></el-form-item>
                  <el-form-item label="个人标题"><el-input v-model="accountForm.headline" maxlength="120" placeholder="如：AI 产品课程负责人 / 助教" /></el-form-item>
                  <el-form-item label="头像链接"><el-input v-model="accountForm.avatar_url" placeholder="https://..." maxlength="512" /></el-form-item>
                  <el-form-item label="个人介绍"><el-input v-model="accountForm.bio" type="textarea" :rows="4" maxlength="240" show-word-limit placeholder="如任课老师、助教、负责班级等" /></el-form-item>
                  <el-form-item label="教学/研究方向"><el-input v-model="accountForm.research_interest" maxlength="240" placeholder="如：智慧教育、项目制学习、AI 产品设计" /></el-form-item>
                  <el-form-item label="可联系时间"><el-input v-model="accountForm.availability" maxlength="120" placeholder="如：周二/周四答疑，24 小时内回复" /></el-form-item>
                  <el-form-item label="课程主页"><el-input v-model="accountForm.portfolio_url" placeholder="https://..." maxlength="512" /></el-form-item>
                  <el-form-item label="公开主页"><el-input v-model="accountForm.github_url" placeholder="https://..." maxlength="512" /></el-form-item>
                  <el-form-item label="主页主题"><el-select v-model="accountForm.display_theme" style="width:100%"><el-option label="Ocean 海蓝" value="ocean" /><el-option label="Aurora 极光" value="aurora" /><el-option label="Sunrise 晨光" value="sunrise" /></el-select></el-form-item>
                  <el-button type="primary" :loading="loading" @click="saveAccount">保存资料</el-button>
                </el-form>
              </div>
              <div class="card">
                <h3>账号安全</h3>
                <p class="hint">修改密码不会强制退出当前会话，下次登录请使用新密码。</p>
                <el-form label-position="top">
      ...
      <el-drawer v-model="profileDrawerVisible" size="720px" title="学员画像详情">
        <template v-if="selectedUserRow?.profile">
          ...
          <div class="detail-section">
            ...
            <div class="breakdown-table">
              <div class="breakdown-head"><span>维度</span><span>主动</span><span>LLM</span><span>被动</span><span>任务</span><span>规则</span></div>
          ...
          <div class="detail-section">
            ...
            <div class="parse-grid">
              <div><span>学历</span><b>{{ selectedUserRow.profile.degree || '—' }}</b></div>
              <div><span>专业/方向</span><b>{{ selectedUserRow.profile.major || selectedUserRow.profile.field || '—' }}</b></div>
              <div><span>偏好角色</span><b>{{ selectedUserRow.profile.pref_role || '—' }}</b></div>
              <div><span>画像时间</span><b>{{ selectedUserRow.profile.create_time || '—' }}</b></div>
            </div>
          </div>
        ...
        <el-empty v-else description="暂无画像详情" />
      </el-drawer>

      <el-dialog v-model="rulesVisible" title="计算规则说明" width="760px">
        <div class="rule-flow">
          <div class="rule-card"><strong>1. 信息整理</strong><p>主动标签、自由描述、社区互动、聊天和任务表现会作为分组参考。</p></div>
          <div class="rule-arrow">→</div>
          <div class="rule-card"><strong>2. 三维评分</strong><p>知识、技能、协作分别按不同权重融合，最终归一到 0-10 分。</p></div>
          <div class="rule-arrow">→</div>
          <div class="rule-card"><strong>3. 分组/任务</strong><p>系统根据角色、标签、技能和工作量生成分组与任务建议。</p></div>
        </div>
        <div class="formula-panel">
          <h4>画像评分公式</h4>
          <p><b>知识</b> = 30%主动标签 + 30%LLM + 15%被动标签 + 15%任务表现 + 10%规则解析</p>
          <p><b>技能</b> = 35%主动标签 + 30%LLM + 15%被动标签 + 20%任务表现</p>
          <p><b>协作</b> = 25%主动标签 + 20%LLM + 30%被动标签 + 25%任务表现</p>
        </div>
        <div class="formula-panel">
          <h4>分组逻辑</h4>
          <p>系统先挑选每组核心成员，再逐步补充其他成员，参考技能差异、标签多样性、角色搭配和专业方向。若组间差距过大，会进行微调。</p>
        </div>
        <div class="formula-panel">
          <h4>任务分配逻辑</h4>
          <p>候选人匹配分 = 角色匹配 + 标签匹配 + 技能适配 - 当前工时负载。没有明显合适人选时，优先分给累计工时较少的成员。</p>
        </div>
      </el-dialog>
    </div>
  `,
